...
export class ResultVisualisation extends React.Component<{
  ...
  render() {
    const { word, query, index, noMargin } = this.props;

    const { height, width, letterHeight, letterWidth, strokeWidth } =
    ...
    return (
      <ResultContainer onClick={this.toggleExpanded}>
        ...
        {expanded && (
          ...
          >
            <ResultVisualisation word={word} query={query} index={index} />
            <span
              style={{
                fontWeight: expanded ? "bold" : "normal",
              }}
...
> {
  ...
  shouldComponentUpdate(
    newProps: AnagramResultProps,
    newState: AnagramResultState
  ) {
    if (
      this.props.counter !== newProps.counter ||
  ...
  render() {
    ...
    const maxHeight =
      MAX_ITEMS_TO_SHOW_AT_ONCE * rowHeight +
      minHeight +
      paddingSize +
      showAllSize;
    ...
    let height: any = useMaxsize
      ? maxHeight
      : minHeight + maxLengthInGroup * rowHeight + paddingSize;
    let useAutoHeight = this.state.showAll;
    if (this.state.expandedSelections) {
      useAutoHeight =
        useAutoHeight ||
        Object.values(this.state.expandedSelections).some((n) => n);
    }
    ...
    return (
      ...
      >
        <div style={{ position: "relative", width: "100%" }}>
          ...
          {take(
            sortedList,
            !this.state.showAll ? MAX_ITEMS_TO_SHOW_AT_ONCE : list.length
          ).map((a, i) => {
            return (
              <Result
                expanded={
                  !!(
                    this.state.expandedSelections &&
                    this.state.expandedSelections[i]
                  )
                }
                key={i}
                result={a}
